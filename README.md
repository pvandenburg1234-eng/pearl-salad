...
| Version | Date | Notes |
|---|---|---|
| v1.4.1 | 2026-09-26 | Log shipping posts at once (and waits up to 5 s for the post) before asking Salad to reallocate, and first thing on SIGTERM. v1.4.0 lost the last seconds of a rejected node's log - the lines that say why it was rejected - because Salad stops the container before the next 10 s post (seen on a 5070 Ti Laptop host whose network cannot complete TLS to the pool). Same scripts as pearl-salad-nvidia v1.6.1. |
